"""JJ-23: Wire EXECUTIVE_SUMMARY_01 constraints into the shared AT-8 compression loop."""
...
from services.slides.summary_constraints import register_summary_constraints
...
SummaryCompressFieldsFn = Callable[
    [OffendingFieldValues, list[ConstraintViolation]],
    OffendingFieldValues,
]


def validate_and_compress_summary_slide_spec(
    slide_spec: dict[str, Any],
    *,
    compress_fields: SummaryCompressFieldsFn,
) -> CompressionResult:
    """Validate an EXECUTIVE_SUMMARY_01 SlideSpec and rewrite only max-length violations."""
    registry = register_summary_constraints(LayoutConstraintRegistry())
...
def _targeted_compressor(compress_fields: SummaryCompressFieldsFn):
    def compress(
        slide_spec: dict[str, Any],
