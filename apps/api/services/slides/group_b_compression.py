"""JJ-14: Wire Group B constraints into the shared AT-8 compression loop."""
...
GroupBCompressFieldsFn = Callable[
    [OffendingFieldValues, list[ConstraintViolation]],
    OffendingFieldValues,
]


def validate_and_compress_group_b_slide_spec(
    slide_spec: dict[str, Any],
    *,
    compress_fields: GroupBCompressFieldsFn,
) -> CompressionResult:
    """Validate a Group B SlideSpec and rewrite only max-length violations.

    ``compress_fields`` receives only a ``path -> value`` map for currently offending
    text fields plus their AT-7 violations, and must return rewritten values keyed by
    those same paths.
    """
...
def _targeted_compressor(compress_fields: GroupBCompressFieldsFn):
    def compress(
        slide_spec: dict[str, Any],
